...
def mailSend(nick: str, to_email: str, subject: str, body: str, type=" "):
    ...
    try:
        ...
        exceptionE(f"{type} email send failed : {e}"); sc = e
    ...
    try:
        ...
        else: log.warning("Sent mail folder copy skipped due to email send failure")
    except Exception as e: exceptionE(f"Sent mail folder copy failed : {e}"); sc = e

    # Send Discord webhook
    try:
        if sc != 200: raise sc
        msg = msg.as_string()
        if len(msg) > 4096: msg = msg[:4096] # limit description length
        webhook = DiscordWebhook(url=glob.config.DISCORD_EMAIL_LOG_WEBHOOK)
        embed = DiscordEmbed(description=msg, color=242424)
        embed.set_author(name=f"InlayoBot Sent {type}email", url=f"https://osu.{glob.config.domain}/u/1", icon_url=f"https://a.{glob.config.domain}/1")
        embed.set_footer(text="via Inlayo!")
        webhook.add_embed(embed)
        webhook.execute()
    except Exception as e: exceptionE(f"Discord webhook send failed! | {e}")
    return sc