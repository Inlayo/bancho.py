...
async def check_session() -> Any:
    required_keys = [
        "authenticated",
        "user_data",
        "flash_data",
    ]  # if any one is missing, initialize session
...
@frontend.route("/")
async def home() -> Any:
    return cast(Any, await render_template("home.html"))


@frontend.route("/forgot_emailchecksend", methods=["POST"])
async def forgot_emaliCheckSend_post() -> Any:
    ...
    if isExistRedisKEY != -2:
        return str(isExistRedisKEY)
    key = "".join(
        random.choices(
    ...
    await glob.redis.set(
        f"guweb:ForgotEmailVerify:{email}",
        key,
        glob.config.SentEmailTimeout,
    )
    mst = mailSend(username, email, "Inlayo Forgot Email Verification", key)
    if mst == 200:
        return "sent"
    else:
        await glob.redis.delete(f"guweb:ForgotEmailVerify:{email}")
        return f"ERROR | {mst}"
...
async def forget_resetpassword() -> Any:
    ...
    try:
        RedisKEY = (await glob.redis.get(f"guweb:ForgotEmailVerify:{email}")).decode(
            "utf-8",
        )
    except:
        return await flash("error", "Email verification code is Expired.", "forgot")
    if emailkey == RedisKEY:
        await glob.redis.delete(f"guweb:ForgotEmailVerify:{email}")
    else:
        return await flash("error", "Email verification code is Incorrect.", "forgot")
...
async def rules() -> Any:
    return cast(Any, await render_template("rules.html"))
...
@login_required
async def settings_profile_emaliCheckSend_post() -> Any:
    omstatus: str | int | None = None
    nmstatus: str | int | None = None
    form = await request.form
    ...
    if old_email != new_email:
        isExistEmail = await glob.db.fetch(
            "SELECT email FROM users WHERE email = %s",
            new_email,
        )
        if isExistEmail:
            return "exist"
        isExistRedisKEY = await glob.redis.ttl(
            f"guweb:Settings/profile:{old_email}->{new_email}",
        )
        if isExistRedisKEY != -2:
            nmstatus = isExistRedisKEY
        else:
            newkey = "".join(
                random.choices(
                    string.ascii_letters + string.digits,
            ...
            mst = mailSend(
                new_name,
                new_email,
                "Inlayo settings/profile new Email Verification",
                f"new Email({new_email}) Verification\n\n{newkey}",
            )
            if mst == 200:
                nmstatus = "sent"
            else:
    ...
    if isExistRedisKEY != -2:
        ...
        mst = mailSend(
            old_name,
            old_email,
            "Inlayo settings/profile old Email Verification",
            f"old Email({old_email}) Verification\n\n{oldkey}",
        )
        await glob.redis.set(
            f"guweb:Settings/profile:{old_email}",
            oldkey,
            glob.config.SentEmailTimeout,
        )
        if mst == 200:
...
@login_required
async def settings_profile() -> Any:
    return cast(Any, await render_template("settings/profile.html"))


@frontend.route("/settings/profile", methods=["POST"])
@login_required
async def settings_profile_post() -> Any:
    form = await request.form
    ...
    log2.debug2(session)
    log2.info(
        f"{old_name}, {new_name} | {old_email}, {new_email} | {oldEmailKey}, {newEmailKey}",
    )
    ...
    # Check old email verification code
    try:
        RedisKEY = (await glob.redis.get(f"guweb:Settings/profile:{old_email}")).decode(
            "utf-8",
        )
    except:
        return await flash(
            "error",
            "old Email verification code is Expired.",
            "settings/profile",
        )
    if oldEmailKey != RedisKEY:
        return await flash(
            "error",
            "old Email verification code is Incorrect.",
            "settings/profile",
        )
    ...
    if new_name != old_name:
        ...
        if new_name.startswith(" ") or new_name.endswith(" ") or "  " in new_name:
            return await flash(
                "error",
                'Username may not start or end with " " or have two spaces in a row.',
                "settings/profile",
            )

        if await glob.db.fetch(
            "SELECT 1 FROM users WHERE safe_name = %s",
            [utils.get_safe_name(new_name)],
        ):
            return await flash(
                "error",
    ...
    if new_email != old_email:
        # Check new email verification code
        try:
            RedisKEY = (
                await glob.redis.get(f"guweb:Settings/profile:{old_email}->{new_email}")
            ).decode("utf-8")
        except:
            return await flash(
                "error",
                "new Email verification code is Expired.",
                "settings/profile",
            )
        if newEmailKey == RedisKEY:
            await glob.redis.delete(f"guweb:Settings/profile:{old_email}->{new_email}")
        else:
            return await flash(
                "error",
                "new Email verification code is Incorrect.",
                "settings/profile",
            )
...
async def topplays() -> Any:
    ...
    scores = await glob.db.fetchall(
        "SELECT s.status, s.id scoreid, userid, pp, mods, grade, m.set_id, m.title, m.version, u.country, u.name "
...
@login_required
async def settings_avatar() -> Any:
    return cast(Any, await render_template("settings/avatar.html"))


@frontend.route("/settings/avatar", methods=["POST"])
@login_required
async def settings_avatar_post() -> Any:
    # constants
...
@login_required
async def settings_custom_post() -> Any:
...
@login_required
async def settings_password() -> Any:
    return cast(Any, await render_template("settings/password.html"))


@frontend.route("/settings/password", methods=["POST"])
@login_required
async def settings_password_post() -> Any:
    form = await request.form
...
async def login() -> Any:
    ...
    return cast(Any, await render_template("login.html"))


@frontend.route("/verify", methods=["GET"])
async def verify() -> Any:
    return cast(Any, await render_template("verify.html"))


@frontend.route("/how")
async def how() -> Any:
    return cast(Any, await render_template("howtoconnect.html"))
...
async def login_post() -> Any:
    ...
    user_info = await glob.db.fetch(
        "SELECT u.id, u.name, u.email, u.priv, u.pw_bcrypt, u.country, u.silence_end, u.donor_end "
...
async def beatmapsetse(sid: str) -> Any:
    mode = request.args.get("mode", "std", type=str)  # 1. key 2. default value
    mods = request.args.get("mods", "vn", type=str)
...
async def score_select(id: str) -> Any:
    mods_mode_strs = {
    ...
    score_data = await glob.db.fetch(
        "SELECT pp, time_elapsed, play_time, score, grade, id, nmiss, n300, n100, n50, acc, userid, mods, max_combo, mode, map_md5 FROM scores WHERE id = %s",
        [id],
    )
    if not score_data:
    ...
    map_data = await glob.db.fetch(
        "SELECT id, total_length, set_id, diff, title, creator, version, artist, status, max_combo FROM maps WHERE md5 = %s",
        [score_data["map_md5"]],
    )
    if not map_data:
...
async def register_emaliCheckSend_post() -> Any:
    form = await request.form
    ...
    await glob.redis.set(
        f"guweb:RegisterEmailVerify:{email}",
        key,
        glob.config.SentEmailTimeout,
    )
    ...
    if mst == 200:
        ...
        await glob.redis.delete(f"guweb:RegisterEmailVerify:{email}")
        return f"ERROR | {mst}"
...
async def register_post() -> Any:
    ...
    # Check email verification code
    try:
        RedisKEY = (await glob.redis.get(f"guweb:RegisterEmailVerify:{email}")).decode(
            "utf-8",
        )
    except:
        return await flash("error", "Email verification code is Expired.", "register")
    if emailkey == RedisKEY:
        await glob.redis.delete(f"guweb:RegisterEmailVerify:{email}")
    else:
        return await flash("error", "Email verification code is Incorrect.", "register")
...
async def get_profile_banner(user_id: int) -> Any:
...
async def get_profile_background(user_id: int) -> Any:
    # Check if avatar exists
    for ext in ("jpg", "jpeg", "png", "gif"):
