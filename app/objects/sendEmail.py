...
def exceptionE(msg: str="") -> str: e = traceback.format_exc(); log.error(f"{msg} \n{e}"); return e
def mailSend(nick: str, to_email: str, subject: str, body: str, type: str=" ", html: bool=False) -> int | Exception:
    ...
    # 디코 웹훅 전송
    try:
        if sc != 200: raise sc # type: ignore
        ...
        else: msg = msg.as_string() # type: ignore
        if len(msg) > 4096: msg = msg[:4096] # type: ignore #description 길이제한 
        embed = Embed(description=msg, color=242424)
        embed.set_author(name=f"BanchoBot Sent {type}email", url=f"https://osu.{app.settings.DOMAIN}/u/1", icon_url=f"https://a.{app.settings.DOMAIN}/1")
